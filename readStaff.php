...
$query = "SELECT Departments.name, GROUP_CONCAT(CONCAT(FName, ' ', LName) SEPARATOR ', ') AS fullName FROM Staff left outer join Departments on Staff.Departments_depart_id = Departments.depart_id GROUP BY Departments.name";
...
if ($stmt) {
    ...
    echo "<table>";
    echo "  <thead>";
    echo "    <tr><th>Department Name</th><th>Staff Name</th><th></th><th></th></tr>";
    echo "  </thead>";
    echo "  <tbody>";
    while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        echo "<td><a href='editStaff.php?staff_id=".urlencode($row["depart_id"])."'>Edit</a></td>";
        echo "<td><a href='deleteStaff.php?staff_id=".urlencode($row["depart_id"])."' onclick='return confirm('Are you sure you want to delete?');'>Delete</a></td>";
        echo "</tr>";
    }
    echo "  </tbody>";
    echo "</table>";
